...
This is still WIP and doesn't work atm
...
(function(window) {
	...
	const effectList = [
		...
		function glitchWave(context,width,height) {
			...
			const imageData = new ImageData(width,cuttingHeight);
		...
		function glitchSlip(context,width,height) {
			...
			for (let h = startHeight; h < endHeight; h++) {
				...
				if (Math.random() < .1) h++;
				let imageData = new ImageData(width, 1);
		...
		function glitchColor(context,width,height) {
			...
			const startHeight  = height * Math.random();
			const endHeight    = startHeight + 30 + (Math.random() * 40);
			const imageData    = new ImageData(width, endHeight);
			const length       = width * height;
	...
	function init() {
        ...
        _canvas.classList.add("glitched-canvas-bg");
